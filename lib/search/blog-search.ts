import type { BlogPost } from '@/lib/services/strapiService';
import {
  boundedLevenshtein,
...
type SectionKey = 'title' | 'summary' | 'content';
...
const createHighlightedSnippet = (text: string, tokens: string[]): HighlightSnippet | null => {
  ...
  if (firstHighlight === -1) {
    ...
    const shortened = trimmed.length > SNIPPET_LENGTH ? `${trimmed.slice(0, SNIPPET_LENGTH).trimEnd()}…` : trimmed;
    return {
      prefix: false,
  ...
  // try to align to word boundaries for readability
...
const indexPosts = (posts: BlogPost[]): IndexedPost[] =>
  posts.map((post) => ({
    ...
    normalizedSummary: buildNormalizedCorpus(post.summary || ''),
    normalizedContent: buildNormalizedCorpus(post.content || ''),
    titleTokens: tokenize(post.title || ''),
    summaryTokens: tokenize(post.summary || ''),
...
const scorePost = (
  ...
  for (const token of queryTokens) {
    ...
    const summaryScore = evaluateSectionMatch(normalizedToken, entry.summaryTokens, entry.normalizedSummary);
    sectionTotals.summary += summaryScore * SECTION_WEIGHTS.summary;

    const contentScore = evaluateSectionMatch(normalizedToken, entry.contentTokens, entry.normalizedContent);
    sectionTotals.content += contentScore * SECTION_WEIGHTS.content;
  }

  if (
    sectionTotals.title === 0 &&
    sectionTotals.summary === 0 &&
    sectionTotals.content === 0
  ) {
    return null;
  }

  let rawScore = sectionTotals.title + sectionTotals.summary + sectionTotals.content;
  if (normalizedQuery) {
    if (entry.normalizedTitle.includes(normalizedQuery)) {
      rawScore += PHRASE_BONUS * 1.25;
      sectionTotals.title += PHRASE_BONUS * 0.5;
    }
    if (entry.normalizedSummary.includes(normalizedQuery)) {
      rawScore += PHRASE_BONUS;
      sectionTotals.summary += PHRASE_BONUS * 0.4;
  ...
  const sections: SectionKey[] = ['title', 'summary', 'content'];
  ...
  const theoreticalMax =
    queryTokens.length * (SECTION_WEIGHTS.title + SECTION_WEIGHTS.summary + SECTION_WEIGHTS.content) +
    PHRASE_BONUS * 1.25 +
    1;
  ...
  const snippetField =
    matchedField === 'title'
      ? entry.post.summary || entry.post.content || ''
      : matchedField === 'summary'
        ? entry.post.summary || entry.post.content || ''
        : entry.post.content || entry.post.summary || '';
